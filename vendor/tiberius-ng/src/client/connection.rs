...
impl<S: AsyncRead + AsyncWrite + Unpin + Send> Connection<S> {
    ...
    // ------------------------------------------------------------------------------------
    // Cobalt patch (see COBALT-PATCH.md)
    // ------------------------------------------------------------------------------------

    /// Writes a TDS Attention packet (type `0x06`, empty payload) and flushes it.
    ...
    /// After an Attention has been sent: discard whatever is buffered (the token stream may
    /// be positioned in the middle of a value), drain the in-flight response on *packet*
    /// boundaries, and make sure the acknowledging `DONE` token (`DONE_ATTN`, always the last
    /// token of a message) has been consumed. If the response had already completed before
    /// the server processed the Attention, the acknowledgement arrives as a message of its
    /// own and is drained as well. Leaves the connection ready for the next request.
    pub(crate) async fn resync_after_attention(&mut self) -> crate::Result<()> {
        self.buf.truncate(0);

        let mut acknowledged = false;
        if !self.flushed {
            acknowledged = self.drain_message_looking_for_attention_ack().await?;
        }
        if !acknowledged {
            self.drain_message_looking_for_attention_ack().await?;
        }

        Ok(())
    }

    /// Reads whole packets up to and including the next end-of-message packet, returning
    /// `true` when the message ended with a `DONE` token carrying `DONE_ATTN`.
    async fn drain_message_looking_for_attention_ack(&mut self) -> crate::Result<bool> {
        // DONE = token type (1) + status (2) + curcmd (2) + row count (4 or 8).
        let done_len = 5 + self.context.version().done_row_count_bytes() as usize;
        // Keep the last `done_len` bytes across packets: a token may straddle a boundary.
        let mut tail: Vec<u8> = Vec::with_capacity(done_len * 2);
        ...
        loop {
            match self.try_next().await {
                Ok(Some(packet)) => {
                    ...
                    if tail.len() > done_len {
                        let drop = tail.len() - done_len;
                        tail.drain(..drop);
                    }

                    if last {
                        let ack = tail.len() == done_len
                            && tail[0] == crate::TokenType::Done as u8
                            && (tail[1] & (crate::tds::codec::DoneStatus::Attention as u16 as u8)) != 0;
                        return Ok(ack);
                    }
                }
                Ok(None) => {
                    self.poisoned = true;
                    return Err(crate::Error::Protocol(
                        "connection closed while waiting for the attention acknowledgement".into(),
                    ));
                }
                Err(e) => {
                    self.poisoned = true;
    ...
    #[allow(dead_code)] // Cobalt patch: superseded by send_attention + resync_after_attention
