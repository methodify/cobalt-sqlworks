...
impl TokenDone {
    ...
    // --- Cobalt patch: extra status accessors ---
    /// `true` when the `DONE_COUNT` bit is set, i.e. `rows()` is a real count.
    pub fn has_count(&self) -> bool { self.status.contains(DoneStatus::Count) }
    /// `true` when more results follow in this response (`DONE_MORE`).
    pub fn is_more(&self) -> bool { self.status.contains(DoneStatus::More) }
    /// `true` when the statement ended in error (`DONE_ERROR` or `DONE_SRVERROR`).
    pub fn has_error(&self) -> bool { self.status.contains(DoneStatus::Error) || self.status.contains(DoneStatus::SrvError) }
    pub fn status(&self) -> BitFlags<DoneStatus> { self.status }
    pub fn cur_cmd(&self) -> u16 { self.cur_cmd }
