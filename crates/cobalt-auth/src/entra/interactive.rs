...
pub(crate) fn authorize_url(
    ...
    let mut url = Url::parse(&cfg.authorize_endpoint())
        .map_err(|e| AuthError::Other(format!("bad authority: {e}")))?;
    {
...
pub async fn interactive_login(
    ...
    tracing::info!(
        port = listener.port(),
        tenant = cfg.tenant(),
        "starting interactive Entra sign-in"
    );
    on_url(url.as_str());
    ...
    let wait = tokio::task::spawn_blocking(move || {
        listener.wait_for_code(&expected_state, INTERACTIVE_TIMEOUT, &cancel_for_task)
    });
    // A belt-and-braces outer timeout, slightly longer than the listener's own deadline.
    let redirect =
        match tokio::time::timeout(INTERACTIVE_TIMEOUT + Duration::from_secs(5), wait).await {
            Ok(Ok(r)) => r?,
...
mod tests {
    ...
    fn authorize_url_has_every_parameter() {
        ...
        assert_eq!(
            q["scope"],
            "https://database.windows.net/.default offline_access openid profile"
        );
    ...
    fn missing_client_id_is_rejected_early() {
        ...
        let e =
            authorize_url(&cfg, "http://localhost:1", "s", &Pkce::generate(), None).unwrap_err();
