...
pub async fn azure_cli_token_for_scope(tenant: Option<&str>, scope: &str) -> Result<AccessToken> {
    let tenant_id = tenant.map(str::trim).filter(|t| !t.is_empty()).map(str::to_owned);
    let cred = AzureCliCredential::new(Some(AzureCliCredentialOptions { tenant_id, ..Default::default() }))
        .map_err(|e| AuthError::AzureCli(e.to_string()))?;
    let tok = cred.get_token(&[scope], None).await.map_err(map_cli_error)?;
    let expires_at = DateTime::<Utc>::from_timestamp(tok.expires_on.unix_timestamp(), 0).unwrap_or_else(Utc::now);
    Ok(AccessToken { token: Secret::new(tok.token.secret()), expires_at, scope: scope.to_owned() })
}
...
pub(crate) fn classify_cli_message(msg: &str) -> String {
    ...
    let not_installed = ["not found", "no such file", "cannot find", "not recognized", "program not found", "os error 2"]
        .iter()
...
mod tests {
    ...
    fn cli_messages_are_classified() {
        ...
        assert!(classify_cli_message("'az' is not recognized as an internal or external command").contains("not found on PATH"));
        assert!(classify_cli_message("AzureCliCredential authentication failed. Please run 'az login' to setup account.").contains("az login"));
    ...
    async fn cli_rejects_bad_tenant_cleanly() {
        ...
        let e = azure_cli_token(Some("not a tenant; rm -rf /")).await.unwrap_err();
