...
use cobalt_auth::SecretStore;
use cobalt_auth::{
    client_secret_token, device_code_login, interactive_login, refresh, AuthError, CancelToken,
    CredentialResolver, EntraConfig, HeadlessPrompter, MemoryStore,
};
...
async fn interactive_flow_end_to_end() {
    ...
    idp.queue_token_ok(
        "access-1",
        Some("refresh-1"),
        Some(&fake_id_token("ada@contoso.com", "Ada Lovelace")),
    );
    ...
    let browser = page.lock().unwrap().take().unwrap();
    ...
    assert_eq!(
        set.account.home_account_id.as_deref(),
        Some("oid-5678.tenant-1234")
    );
...
async fn interactive_rejects_state_mismatch() {
    ...
    let err = interactive_login(
        ...
        None,
        move |url| {
            *p2.lock().unwrap() = Some(spawn_browser(url, |_state| {
                "code=fake&state=WRONG".to_owned()
            }));
        },
        CancelToken::new(),
    )
    ...
    assert!(
        idp.token_requests().is_empty(),
        "no code exchange on a bad state"
    );
}
...
async fn interactive_honours_fixed_redirect_port() {
    ...
    assert!(
        seen.lock()
            .unwrap()
            .contains(&format!("localhost%3A{port}")),
        "{}",
        seen.lock().unwrap()
    );
}
...
async fn refresh_rotates_token_and_decodes_account() {
    ...
    idp.queue_token_ok(
        "access-2",
        Some("refresh-2"),
        Some(&fake_id_token("bob@contoso.com", "Bob")),
    );
    let cfg = idp.config("test-client");
...
async fn refresh_uses_token_endpoint_override() {
    ...
    let cfg = EntraConfig {
        ...
        token_endpoint_override: Some(
            url::Url::parse(&format!("{}/custom/token", idp.base)).unwrap(),
        ),
        ..Default::default()
    };
    assert_eq!(
...
async fn network_failure_is_an_http_error() {
    ...
    let cfg = EntraConfig {
        client_id: "c".into(),
        authority_host: format!("http://127.0.0.1:{port}"),
        ..Default::default()
    };
...
async fn device_code_flow_pending_then_success() {
    ...
    idp.queue_token_ok(
        "access-dc",
        Some("refresh-dc"),
        Some(&fake_id_token("cy@contoso.com", "Cy")),
    );
    let cfg = idp.config("test-client");
    let prompts = Arc::new(Mutex::new(Vec::new()));
    let p2 = prompts.clone();
    let set = device_code_login(
        &cfg,
        move |p| p2.lock().unwrap().push(p.clone()),
        CancelToken::new(),
    )
    .await
    .unwrap();
...
async fn device_code_declined_and_expired() {
    ...
    idp.queue_token_error(
        ...
        "AADSTS70020: The end user has denied the authorization request.",
    );
    let cfg = idp.config("test-client");
    let err = device_code_login(&cfg, |_| {}, CancelToken::new())
        .await
        .unwrap_err();
    match &err {
...
async fn device_code_request_error_is_mapped() {
    ...
    let cfg = idp.config("test-client");
    let err = device_code_login(&cfg, |_| panic!("no prompt on failure"), CancelToken::new())
        .await
        .unwrap_err();
...
async fn client_secret_flow_posts_client_credentials() {
    ...
    let t = client_secret_token(
        &cfg,
        "11111111-2222-3333-4444-555555555555",
        "sp-client",
        &Secret::new("sp-secret"),
    )
    .await
    .unwrap();
...
async fn client_secret_error_is_provider_error() {
    ...
    let err = client_secret_token(&cfg, "contoso.com", "sp", &Secret::new("bad"))
        .await
        .unwrap_err();
    assert!(matches!(err, AuthError::Provider { .. }), "{err:?}");
}
...
fn entra_profile() -> ConnectionProfile {
    ConnectionProfile::new(
        ...
        AuthMethod::EntraInteractive {
            tenant: None,
            account_hint: Some("ada@contoso.com".into()),
        },
    )
}
...
async fn resolver_policy_cache_then_refresh_then_browser() {
    ...
    idp.queue_token_ok(
        "access-r1",
        Some("rt-new"),
        Some(&fake_id_token("ada@contoso.com", "Ada")),
    );
    let prompt = HeadlessPrompter::new()
        .with_on_open(|_| panic!("browser must not open on a silent refresh"));
    assert_eq!(
        token_of(resolver.resolve(&profile, &prompt).await.unwrap()),
        "access-r1"
    );
    assert_eq!(
        store.get(&rt_ref).unwrap().unwrap().expose(),
        "rt-new",
        "rotated refresh token persisted"
    );
    assert_eq!(
        resolver.signed_in_account(&profile.id).unwrap().username,
        "ada@contoso.com"
    );
...
async fn resolver_goes_straight_to_browser_when_nothing_stored() {
    ...
    assert_eq!(idp.token_requests().len(), 1);
    assert_eq!(
        idp.token_requests()[0].form["grant_type"],
        "authorization_code"
    );
    assert!(resolver.is_remembered(&profile.id));
}
...
async fn resolver_device_code_policy() {
    ...
    idp.queue_token_ok(
        "access-dc",
        Some("rt-dc"),
        Some(&fake_id_token("dee@contoso.com", "Dee")),
    );
    let store = Arc::new(MemoryStore::new());
    let resolver = CredentialResolver::new(store.clone(), idp.config("test-client"));
    let profile = ConnectionProfile::new("srv", AuthMethod::EntraDeviceCode { tenant: None });
    let prompt =
        HeadlessPrompter::new().with_on_open(|_| panic!("device code must not open a browser"));
    assert_eq!(
        token_of(resolver.resolve(&profile, &prompt).await.unwrap()),
        "access-dc"
    );
    ...
    store
        .set(
            &CredentialResolver::refresh_token_ref(&profile.id),
            &Secret::new("rt-dc"),
        )
        .unwrap();
...
async fn resolver_cancel_via_prompter() {
    ...
    let started = Instant::now();
    let err = resolver
        .resolve(&profile, prompt.as_ref())
        .await
        .unwrap_err();
    assert!(matches!(err, AuthError::Cancelled), "{err:?}");
    assert!(started.elapsed() < Duration::from_secs(5));
    assert_eq!(
        prompt.opened_urls().len(),
        1,
        "the browser was opened before the cancel"
    );
}
...
async fn resolver_refresh_network_error_is_not_swallowed() {
    ...
    store
        ...
        .unwrap();
    let prompt = HeadlessPrompter::new()
        .with_on_open(|_| panic!("offline: must not fall through to the browser"));
    let err = resolver.resolve(&profile, &prompt).await.unwrap_err();
    assert!(matches!(err, AuthError::Http(_)), "{err:?}");
    assert!(
        resolver.is_remembered(&profile.id),
        "refresh token kept for when the network is back"
    );
}
...
async fn resolver_service_principal_uses_stored_secret_or_prompt() {
    ...
    assert_eq!(
        idp.token_requests()[0].form["client_secret"],
        "stored-secret"
    );
    assert_eq!(prompt.password_prompt_count(), 0);

    profile.auth = AuthMethod::EntraServicePrincipal {
        tenant: "contoso.com".into(),
        client_id: "sp".into(),
        secret: None,
    };
    ...
    assert_eq!(
        idp.token_requests()[1].form["client_secret"],
        "typed-secret"
    );
